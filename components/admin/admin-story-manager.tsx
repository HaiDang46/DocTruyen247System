...
import { chapters, stories as mockStories, type Chapter, type Story } from "@/lib/mock-data";
import { parseSingleTxt, parseEpubStory, parseZipStory, convertHtmlToText, extractChapterNumber } from "@/lib/story-importer";
import {
  isDbConnected,
  getStoriesDb,
  getChaptersDb,
...
function createStoryFromForm(form: StoryFormState, index: number): Story {
...
function toStoryForm(story: Story): StoryFormState {
  return {
    title: story.title,
    author: story.author,
    type: story.type,
...
export function AdminStoryManager() {
  ...
  async function loadChaptersForStory(storyId: string) {
    ...
    isDbConnected().then((connected) => {
      ...
      if (connected) {
        ...
        if (savedChapters) {
          ...
          setManagedChapters(chapters.map((chapter) => ({ ...chapter, storyId: mockStories[0]?.id ?? "" })));
        }
      }
    });
  }, []);
  ...
  useEffect(() => {
    if (isMounted && !dbConnected) {
      localStorage.setItem("doc_truyen_chapters", JSON.stringify(managedChapters));
    }
  ...
  const selectedStory = useMemo(
    () => filteredStories.find((story) => story.id === selectedStoryId) ?? stories.find((story) => story.id === selectedStoryId) ?? stories[0],
  ...
  const stats = useMemo(
    () => [
      ...
      {
        label: "Đang ra",
        value: String(stories.filter((story) => story.status === "Ongoing").length),
      },
    ],
    [stories]
  );
  ...
  async function handleStorySubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const nextStory: Story = {
      ...
      latestChapter: editingStoryId 
        ? (stories.find(s => s.id === editingStoryId)?.latestChapter || "Chưa có")
    ...
    if (dbConnected) {
      ...
      if (editingStoryId) {
        ...
        if (res.success) {
          setStories((prev) => prev.map((s) => (s.id === editingStoryId ? nextStory : s)));
        ...
        if (res.success && res.data) {
          const dbStory = { ...nextStory, id: res.data.id, slug: res.data.slug };
  ...
  async function handleDeleteStory(storyId: string) {
    ...
    const confirmed = window.confirm(`Bạn có chắc muốn xoá truyện "${story?.title ?? "này"}" cùng tất cả chương truyện không?`);
    ...
    if (dbConnected) {
      ...
      if (res.success) {
        setStories((currentStories) => {
          const nextStories = currentStories.filter((item) => item.id !== storyId);
  ...
  async function handleChapterSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (dbConnected) {
      ...
      if (editingChapterId) {
        ...
        setImportLoading(false);
        if (res.success && res.data) {
          const nextChapter: ManagedChapter = {
            id: res.data.id,
            storyId: selectedStory.id,
            number: chapterNumber,
            title: chapterForm.title.trim(),
          ...
          setManagedChapters((currentChapters) => [nextChapter, ...currentChapters]);
          setStories((currentStories) =>
    ...
    if (editingChapterId) {
      ...
      setChapterForm({
        ...
        title: selectedStory.type === "NOVEL" ? `Chương ${nextNumber}` : `Tập ${nextNumber}`,
        isPremium: false,
        content: "",
        imageUrls: [],
        imageNames: [],
  ...
  function handleEditChapter(chapter: ManagedChapter) {
    ...
    setChapterForm({
      ...
      uploadNote: chapter.imageUrls?.length ? `Đã nạp ${chapter.imageUrls.length} ảnh.` : "",
    });
  }
  ...
  async function handleTextFileUpload(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (imageExtensions.has(extension ?? "")) {
        throw new Error(`Tệp tin "${file.name}" là tệp tin hình ảnh. Vui lòng tải các file văn bản chữ (.txt, .html, .epub, .zip) cho Truyện chữ.`);
      }

      if (extension === "epub") {
        const imported = await parseEpubStory(file);
        content = imported.chapters.map(c => `${c.title}\n\n${c.content}`).join("\n\n---\n\n");
        ...
        const isHtml = file.name.endsWith(".html") || file.name.endsWith(".htm") || file.name.endsWith(".xhtml") || file.name.endsWith(".xml") || file.type === "text/html" || content.trim().startsWith("<");
        if (isHtml) {
      ...
      if (lines.length > 0) {
        ...
        if (chapterRegex.test(firstLine)) {
          ...
          if (firstLineIndex !== -1) {
            content = content.substring(firstLineIndex + firstLine.length).trim();
          }
        }
      }
  ...
  async function handleMangaImagesUpload(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (files.length === 1 && (extension === "zip" || extension === "cbz")) {
        ...
        const imageEntries = Object.values(zip.files)
          .filter((entry) => !entry.dir && isSupportedMangaImageName(entry.name))
  ...
  async function handleDeleteChapter(chapterId: string) {
    const confirmed = window.confirm("Bạn có chắc chắn muốn xóa chương này khỏi hệ thống?");
    if (!confirmed) return;
  ...
  async function handleFileImport(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const imageExtensions = new Set(["jpg", "jpeg", "png", "gif", "webp", "bmp", "svg", "avif"]);
      if (imageExtensions.has(extension ?? "")) {
        throw new Error(`Tệp tin "${file.name}" là tệp tin hình ảnh. Vui lòng tải tệp tin văn bản chữ (.txt, .epub, .zip) hoặc tệp nén truyện tranh (.zip chứa ảnh, .cbz).`);
      }

      const textExtensions = new Set(["txt", "html", "htm", "xhtml", "xml", "md", "json", "jsonl"]);
      if (textExtensions.has(extension ?? "")) {
        setImportMessage(`Đang đọc và phân tích file ${extension?.toUpperCase()}...`);
        let text = await new Promise<string>((resolve, reject) => {
          const reader = new FileReader();
          reader.onload = () => resolve(String(reader.result ?? ""));
          reader.onerror = () => reject(new Error(`Lỗi đọc file ${extension?.toUpperCase()}`));
          reader.readAsText(file, "utf-8");
        });
        ...
        try {
          ...
          throw new Error(`Không hỗ trợ định dạng file .${extension}. Vui lòng sử dụng các định dạng văn bản hoặc file nén zip/epub/cbz.`);
        }
      }
      ...
      if (dbConnected) {
        ...
        const storyRes = await createStoryDb({
          ...
          type: imported.type,
          status: "Ongoing",
          tags: imported.type === "NOVEL" ? ["Kỳ ảo", "Bí ẩn"] : ["Hành động", "Phiêu lưu"],
        ...
        if (storyRes.success && storyRes.data) {
          ...
          setImportMessage(`Đang lưu ${imported.chapters.length} chương vào database...`);
        ...
        const newStorySlug = slugify(imported.title) || `truyen-nhap-${Date.now()}`;
        ...
        const nextStory: Story = {
          ...
          rating: 4.8,
          views: "0",
          latestChapter: imported.type === "NOVEL" 
        ...
        // Create chapters
        const nextChapters: ManagedChapter[] = imported.chapters.map((chap: any, idx: number) => ({
          id: `chapter-local-${Date.now()}-${idx}`,
          storyId: newStoryId,
          number: chap.number || (idx + 1),
          title: chap.title || (imported.type === "NOVEL" ? `Chương ${idx + 1}` : `Tập ${idx + 1}`),
          isPremium: false,
          content: chap.content,
          imageUrls: chap.imageUrls,
          imageNames: chap.imageNames,
    ...
    <div className="space-y-6">
      ...
      <div className="surface-panel p-3 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-2 px-2">
          ...
          <span className="text-sm font-black uppercase text-ink tracking-wider">Hệ thống quản trị</span>
        </div>
      ...
      <main className="space-y-6">
        ...
        {activeMenu === "Truyện" && (
          <section className="grid gap-6 lg:grid-cols-[1fr_380px]">
            <div className="surface-panel p-4 min-w-0">
              <SectionHeader title="Danh sách truyện" action={`${filteredStories.length} truyện`} />
                    ...
                    {filteredStories.map((story) => (
                      <tr
                        ...
                        className={`transition hover:bg-muted/60 ${
                          selectedStory?.id === story.id ? "bg-primarySoft/40" : ""
                        }`}
                      >
                        <td className="py-3 px-4">
                          ...
                          <p className="mt-1 text-xs text-subtle">{story.author}</p>
                        </td>
            ...
            <div className="space-y-6">
              <div className="surface-panel p-4">
                ...
                <form className="mt-4 space-y-3" onSubmit={handleStorySubmit}>
                  <input
                    ...
                    onChange={(event) =>
                      setStoryForm((form) => ({ ...form, title: event.target.value }))
                    }
                  />
                  <input
                    className="soft-control w-full px-3 py-2.5 text-sm outline-none"
                    ...
                    onChange={(event) =>
                      setStoryForm((form) => ({ ...form, author: event.target.value }))
                    }
                  />
                  <div className="grid gap-3 sm:grid-cols-2">
                    <select
                      ...
                      onChange={(event) =>
                        setStoryForm((form) => ({
                          ...form,
                          type: event.target.value as Story["type"],
                        }))
                      }
                    >
              ...
              <div className="surface-panel p-4">
                ...
                {selectedStory ? (
                  <div className="mt-4 grid grid-cols-[80px_1fr] gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 text-xs text-subtle">
                        {formatStoryType(selectedStory.type)} · {formatStoryStatus(selectedStory.status)}
                      </p>
                  ...
                  <p className="mt-4 text-sm text-subtle">Chưa có truyện nào.</p>
                )}
              </div>
            </div>
          </section>
              ...
              <div className="mt-4 divide-y divide-line">
                {selectedChapters.length > 0 ? (
                  selectedChapters.map((chapter) => (
                    <div key={chapter.id} className="flex items-center gap-3 py-2.5">
                      <div className="min-w-0 flex-1">
                        <ChapterListItem chapter={chapter} />
                        {chapter.content ? (
                          <p className="pb-1 text-xs font-semibold text-subtle">
                            Đã nhập {chapter.content.length.toLocaleString("vi-VN")} ký tự nội dung
                          </p>
                        ) : null}
                        {chapter.imageUrls?.length ? (
                          <div className="grid max-w-xs grid-cols-4 gap-1.5 pb-1">
                            {chapter.imageUrls.slice(0, 4).map((imageUrl, index) => (
                              <div
                                key={imageUrl}
                              ...
                              >
                                <img
                                  src={imageUrl}
                                  alt={chapter.imageNames?.[index] ?? `Trang ${index + 1}`}
                          ...
                          {chapterForm.imageUrls.map((imageUrl, index) => (
                            ...
                            >
                              <div className="flex w-full justify-between items-center px-1 text-[10px] text-subtle font-bold border-b border-line pb-1">
                                ...
                                <span className="truncate max-w-[200px]" title={chapterForm.imageNames[index]}>
                                  {chapterForm.imageNames[index] ?? "Đang tải..."}
                                </span>
                              </div>
                              <img
                                src={imageUrl}
                                alt={chapterForm.imageNames[index] ?? `Trang ${index + 1}`}
        ...
        {activeMenu === "Tải lên" && (
          <div className="surface-panel p-6 space-y-6">
            <div className="text-center max-w-xl mx-auto space-y-3 py-6">
              ...
              <h2 className="text-xl font-black text-ink">Nhập Truyện Tự Động Từ File</h2>
              <p className="text-xs text-subtle leading-relaxed">
                Hệ thống tự động phân tích và tạo truyện cùng đầy đủ các chương bên trong hoàn toàn ở phía client. Hỗ trợ cả truyện chữ (.txt, .epub) và truyện tranh (.zip).
              </p>
            </div>
            ...
            {importSuccess ? (
              <div className="max-w-xl mx-auto rounded-lg border border-emerald-200 bg-emerald-50 dark:border-emerald-950 dark:bg-emerald-950/20 p-5 space-y-4">
                <div className="flex gap-3">
                  ...
                  <div>
                    <h3 className="text-sm font-black text-emerald-800 dark:text-emerald-400">Nhập truyện thành công!</h3>
                    <p className="mt-1 text-xs text-emerald-700 dark:text-emerald-500">
                      Đã tạo truyện <strong>{importSuccess.title}</strong> ({importSuccess.type === "NOVEL" ? "Truyện chữ" : "Manga"}) với <strong>{importSuccess.chaptersCount} chương/tập</strong>.
                    </p>
                  </div>
                </div>
                <div className="flex gap-2">
              ...
              <div className="max-w-xl mx-auto">
                {importLoading ? (
                  <div className="rounded-lg border border-line bg-canvas p-8 text-center space-y-4">
                    ...
                    <p className="text-sm font-bold text-ink">{importMessage}</p>
                    <p className="text-xs text-subtle">Vui lòng không đóng trình duyệt...</p>
                  </div>
                ) : (
                  <label className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-line bg-canvas p-10 cursor-pointer hover:border-primary transition group text-center">
                    ...
                    <p className="mt-4 text-sm font-black text-ink">Chọn tệp tin để nhập truyện</p>
                    <p className="mt-1 text-xs text-subtle">
                      Hỗ trợ mọi định dạng tệp tin văn bản hoặc lưu trữ nén (.txt, .html, .epub, .zip, .cbz,...)
                    </p>
                    <input
                      type="file"
            ...
            <div className="max-w-2xl mx-auto rounded-lg border border-line bg-muted/40 p-4 space-y-3">
              <h3 className="text-xs font-black uppercase text-subtle tracking-wider">Hướng dẫn chuẩn bị file nhập</h3>
              <div className="grid gap-4 sm:grid-cols-3 text-xs leading-relaxed text-subtle">
                <div className="space-y-1">
                  <p className="font-bold text-ink text-xs">📝 Tệp văn bản đơn lẻ</p>
                  <p>
                    Hỗ trợ .txt, .html, .htm, .md, .json,... Các chương bắt đầu bằng dòng riêng như <strong>Chương [Số]</strong>. Tệp HTML/XML sẽ được tự động lọc sạch thẻ tag khi nhập.
                  </p>
                </div>
                <div className="space-y-1">
                  <p className="font-bold text-ink text-xs">📘 Sách điện tử .EPUB</p>
                  <p>
                    Định dạng chuẩn nhất cho truyện chữ Novel. Hệ thống tự giải nén để lấy ảnh bìa, tên truyện, tác giả, mô tả giới thiệu và toàn bộ cấu trúc mục lục chương một cách chính xác.
                  </p>
                </div>
                <div className="space-y-1">
                  <p className="font-bold text-ink text-xs">📦 File nén .ZIP</p>
                  <p>
                    <strong>Manga:</strong> ZIP chứa các thư mục tương ứng với từng chương (ví dụ: <code>Chương 1</code>, <code>Chương 2</code>), bên trong chứa các file ảnh (.jpg, .png, .webp).
                    <br />
                    <strong>Novel:</strong> ZIP chứa nhiều file text <code>.txt</code> tương ứng với từng chương.
                  </p>
                </div>
              </div>
            </div>
        ...
        {activeMenu === "Người dùng" && (
          <div className="surface-panel p-6 text-center space-y-4">
            ...
            <p className="text-xs text-subtle max-w-sm mx-auto">
              Khu vực quản lý danh sách thành viên, phân quyền độc giả, người dịch và kiểm duyệt viên trên hệ thống.
            </p>
            <div className="rounded-lg border border-line bg-canvas p-4 text-xs font-bold text-subtle">
              Hệ thống đang đồng bộ cơ sở dữ liệu... chức năng này sẽ sớm hoạt động.
            </div>
          </div>
        )}
        ...
        {activeMenu === "Thống kê" && (
          <div className="surface-panel p-6 text-center space-y-4">
            <div className="text-3xl">📊</div>
            <h2 className="text-lg font-black text-ink">Thống Kê Doanh Thu & Lượt Đọc</h2>
            <p className="text-xs text-subtle max-w-sm mx-auto">
              Báo cáo tổng quan về doanh thu bán chương VIP, lượt xem truyện, xu hướng đọc và tương tác của thành viên.
            </p>
            <div className="rounded-lg border border-line bg-canvas p-4 text-xs font-bold text-subtle">
