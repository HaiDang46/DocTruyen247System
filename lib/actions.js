...
// 11. Comment Actions
export async function getCommentsDb(storyId) {
  try {
...
export async function createCommentDb(storyId, userId, content) {
  try {
    ...
    for (const h of history) {
      ...
      result.push({
        story: {
          ...
          title: h.story.title,
          slug: h.story.slug,
          type: h.story.contentType.toUpperCase(),
          status: h.story.status.charAt(0).toUpperCase() + h.story.status.slice(1),
