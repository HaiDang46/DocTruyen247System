...
export async function getStoriesDb() {
  ...
  try {
    ...
    let stories = res.recordset.map(row => ({
      ...
      status: row.status,
      type: row.content_type ? row.content_type.toUpperCase() : "NOVEL",
...
export async function createStoryDb(data) {
  try {
    ...
    const storyRes = await pool.request()
      ...
      .input('type', sql.NVarChar, data.contentType || 'novel')
...
export async function getReadingHistoryDb(userId) {
  ...
  try {
    ...
    let items = res.recordset.map(row => ({
      ...
      progress: Math.min(100, Math.max(0, Math.floor(row.scroll_position / 10))), // Mocking progress 0-100 based on scroll pos
