...
function saveBase64Image(base64Str, folderPath) {
  // Bỏ qua việc lưu file vào ổ cứng, trả thẳng chuỗi Base64 để lưu vào Database
  return base64Str;
}
...
export async function deleteReadingHistoryDb(userId, storyId) {
  try {
    const pool = await getDbConnection();
    await pool.request()
      .input('userId', sql.UniqueIdentifier, userId)
      .input('storyId', sql.UniqueIdentifier, storyId)
      .query('DELETE FROM reading_progress WHERE user_id = @userId AND story_id = @storyId');
      
    await pool.request()
      .input('userId', sql.UniqueIdentifier, userId)
      .input('storyId', sql.UniqueIdentifier, storyId)
      .query('DELETE FROM reading_history WHERE user_id = @userId AND story_id = @storyId');
...
export async function clearReadingHistoryDb(userId) {
  try {
    const pool = await getDbConnection();
    await pool.request()
      .input('userId', sql.UniqueIdentifier, userId)
    ...
    await pool.request()
      .input('userId', sql.UniqueIdentifier, userId)
      .query('DELETE FROM reading_history WHERE user_id = @userId');
...
export async function updateUserDb(userId, name, oldPassword, newPassword) {
  try {
    const pool = await getDbConnection();
    ...
    if (newPassword) {
      if (!oldPassword) {
        return { success: false, error: "Vui lòng nhập mật khẩu cũ để đổi mật khẩu mới." };
      }
      ...
      if (checkRes.recordset[0].password_hash !== oldPassword) {
    ...
    const req = pool.request()
      ...
      .input('name', sql.NVarChar, name);
    if (newPassword) {
      req.input('password', sql.NVarChar, newPassword);
    }
    await req.query(query);
    
    // fetch new user
    const res = await pool.request().input('id', sql.UniqueIdentifier, userId).query('SELECT id, name, email, avatar as avatarUrl, role, created_at as createdAt FROM users WHERE id = @id');
    const user = res.recordset[0];
    return { success: true, data: { id: user.id, email: user.email, name: user.name, avatar: user.avatarUrl, role: user.role } };
  } catch (error) {
    console.error(error);
    return { success: false, error: error.message };
  }
...
export async function getUserStatsDb(userId) {
  try {
    const pool = await getDbConnection();
    const historyRes = await pool.request().input('userId', sql.UniqueIdentifier, userId).query('SELECT COUNT(DISTINCT story_id) as count FROM reading_progress WHERE user_id = @userId');
    const commentRes = await pool.request().input('userId', sql.UniqueIdentifier, userId).query('SELECT COUNT(*) as count FROM comments WHERE user_id = @userId AND deleted_at IS NULL');
    const favRes = await pool.request().input('userId', sql.UniqueIdentifier, userId).query('SELECT COUNT(*) as count FROM favorites WHERE user_id = @userId');
    
    return {
      success: true,
      data: {
        readStories: historyRes.recordset[0].count,
        totalComments: commentRes.recordset[0].count,
        totalFavorites: favRes.recordset[0].count
      }
    };
  } catch (error) {
    console.error(error);
