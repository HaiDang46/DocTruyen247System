...
export default function ProfilePage() {
  const { user } = useAuth();
  const [historyItems, setHistoryItems] = useState<any[]>(mockProfileItems);
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-lg border border-line bg-surface p-4 shadow-soft">
        <SectionHeader title="Đọc tiếp" action={`${historyItems.length} bộ truyện`} />
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          {historyItems.length > 0 ? (
            ...
            <p className="col-span-2 py-12 text-sm text-subtle text-center">Bạn chưa đọc chương nào. Hãy khám phá và bắt đầu đọc ngay nhé!</p>
          )}
        </div>
      </section>
    </div>
